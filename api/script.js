...
export default async function handler(request, response) {
  ...
  const { message, projectContext, researchContext, scriptType, creationMode, manualDraft } = request.body || {};
  if (typeof message !== 'string' || !message.trim()) {
    return response.status(400).json({ error: 'Please enter a production brief.' });
  }
  if (message.length > 8000) return response.status(413).json({ error: 'The request is too long.' });
  if (typeof manualDraft === 'string' && manualDraft.length > 30000) return response.status(413).json({ error: 'The manual draft is too long.' });
  ...
  try {
    const result = await runScriptAgent({ message: message.trim(), projectContext, researchContext, scriptType, creationMode, manualDraft });

...
function renderInspectorDetails(node) {
  ...
  const config = {
    ...
    animation: { steps:['Studio mode','Writer & design team','Scene production','Director cut'], task:'AI Anime StudioでShort / Series制作を管理' },
...
function animeWorkspaceMarkup(result) {
  ...
  const mode = result?.animeBrief?.productionMode || (treatment ? 'quick' : 'quick');
  const team = [
    ['✦','Producer','Scope · schedule · budget'],['✎','Writer','Story · dialogue · rhythm'],['◇','Character Designer','Look · expression · continuity'],
    ['▧','Background Designer','Location · light · props'],['◉','Voice Director','Cast · emotion · timing'],['▷','Motion Agent','Movement · camera · lip sync'],['★','Director','Scene review · final cut']
  ];
  const modeCards = [
    ['quick','⚡','Quick Short Anime','1 Characterから15–60秒の小劇場を高速制作','Character · Idea · Voice'],
    ['series','▦','Series / Long-form','固定Character・Location・VoiceでEpisodeを継続制作','IP Assets · Episodes · Continuity'],
    ['continue','↻','Continue Existing Series','既存IPとEpisode Libraryを開いて次の話を制作','Resume · Reuse · Version'],
    ['import','⇢','Import Existing Project','Research・Script・Assetsを任意で接続して開始','Optional Project Connection']
  ];
  ...
  return `<div class="anime-studio-board">
    <section class="anime-studio-home"><div class="anime-studio-hero"><div><small>TEGY · AI ANIME STUDIO</small><h2>What do you want to create?</h2><p>広告工程から独立して制作できます。必要な場合だけProject Script・Research・Assetsを接続します。</p></div><span>Studio mode · ${escapeHtml(mode)}</span></div><form id="animeWorkspaceForm" class="anime-studio-brief"><div class="anime-entry-modes">${modeCards.map(card=>`<label><input type="radio" name="productionMode" value="${card[0]}" ${mode===card[0]?'checked':''}><span><i>${card[1]}</i><b>${card[2]}</b><small>${card[3]}</small><em>${card[4]}</em></span></label>`).join('')}</div><div class="anime-studio-inputs"><label>Creation Source<select name="mode"><option value="auto">Writer Agent Auto</option><option value="manual">Existing Story / Script</option><option value="import" ${imported?'selected':''}>Connect Project Script</option></select></label><label>Duration<select name="durationSeconds"><option value="15">15 sec</option><option value="30" ${!treatment||treatment.targetDurationSeconds===30?'selected':''}>30 sec</option><option value="45">45 sec</option><option value="60">60 sec</option><option value="180">3 min</option><option value="300">5 min</option></select></label><label>Format<select name="aspectRatio"><option>9:16</option><option>16:9</option><option>1:1</option></select></label><label>Visual Style<select name="style"><option>2D Anime · Character Comedy</option><option>Motion Comic · Limited Animation</option><option>Cinematic Anime</option><option>Chibi / SD Character</option><option>Manga Monochrome</option><option>Soft Illustration</option></select></label><label>Story Type<select name="storyTemplate"><option>Comedy / Reversal</option><option>Daily Life</option><option>Character Introduction</option><option>Fantasy / Ability</option><option>Romance</option><option>Mystery / Suspense</option><option>Educational Story</option><option>Music Video</option></select></label><label>Main Cast<select name="castSize"><option value="1">1 Character</option><option value="2" selected>2 Characters</option><option value="3">3 Characters</option><option value="4">4+ Characters</option></select></label><label>Voice Language<select name="voiceLanguage"><option>Japanese</option><option>English</option><option>No dialogue</option></select></label><label>Voice Direction<select name="voiceDirection"><option>Natural Character Voice</option><option>Anime Expressive</option><option>Comedy Timing</option><option>Calm Narration</option><option>Dramatic</option></select></label></div><label>Creative Note（Optional）<textarea name="requirement" placeholder="選択項目以外に必要なStory、Character、必須Sceneだけ入力してください。空欄でも生成できます。"></textarea></label><label>Existing Script / Series Note（Import時のみ）<textarea name="manualScript" placeholder="既存Script、Character設定、前Episodeの続きがある場合だけ貼り付けます。"></textarea></label><button type="submit">Start Studio Team →</button></form><div class="anime-studio-team"><header><div><small>STUDIO TEAM</small><h3>Specialist Agents</h3></div><span>Director coordinates all outputs</span></header><div>${team.map(member=>`<article><i>${member[0]}</i><b>${member[1]}</b><small>${member[2]}</small><em>${treatment?'Synced':'Waiting'}</em></article>`).join('')}</div></div></section>
    ...
    <section class="anime-voice-motion">${treatment ? `<header><div><small>VOICE DIRECTOR & MOTION AGENT</small><h2>Performance Queue</h2><p>Voice、emotion、lip sync、camera motionをScene単位で確認します。</p></div></header><div class="anime-video-queue">${shots.map(shot=>`<article><b>${escapeHtml(shot.shotNumber)}</b><span>${images[animeImageKey(result,shot.shotNumber)]?'Ready for animation':'Waiting for visual'}</span><button disabled>Send to Video</button></article>`).join('')}</div>` : `<div class="studio-room-empty"><span>◉</span><h2>Voice & Motion</h2><p>Character VoiceとScene Visualが揃うと、Motion Agentへ送信できます。</p></div>`}</section>
    <section class="anime-director-cut">${treatment ? `<div class="director-review"><small>DIRECTOR AGENT</small><h2>Build Anime</h2><p>Character continuity、Scene flow、Voice timing、Motion statusを確認して一本のAnimeへ組み立てます。</p><div><span><b>${shots.length}</b> Scenes</span><span><b>${Object.keys(images).length}</b> Visuals</span><span><b>${Object.keys(generatedVideos).filter(key=>key.startsWith(selectedProject+':')).length}</b> Motion clips</span></div><button ${shots.every(shot=>generatedVideos[`${selectedProject}:${shot.shotNumber}`])?'':'disabled'}>Build Director Cut →</button></div>` : `<div class="studio-room-empty"><span>★</span><h2>Director Cut</h2><p>すべてのSceneをDirector Agentがreviewし、順序・尺・continuityを確定します。</p></div>`}</section>
    <section class="anime-studio-delivery"><div class="studio-delivery-card"><small>ANIME DELIVERY</small><h2>${treatment ? escapeHtml(treatment.title) : 'No production yet'}</h2><p>MP4、Scene package、Character / Asset Library、Subtitle、Voice filesをまとめて納品します。</p><div><button ${treatment?'':'disabled'}>Export Anime Package</button><button ${treatment?'':'disabled'}>Convert Quick Short to Series</button></div></div></section>
  </div>`;
}

function legacyAnimeWorkspaceMarkup(result) {
  const imported = outputs[selectedProject]?.at(-1);
  const treatment = result?.treatment;
  const shots = animeShotList(result);
  const images = storyboardImages[selectedProject] || {};
...
async function runAnimeWorkspace() {
  ...
  $('workspaceSaveStatus').textContent = '● Anime Studio Team running...';
  try {

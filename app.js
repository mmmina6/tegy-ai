...
function animeWorkspaceMarkup(result) {
  ...
  return `<div class="anime-studio-board">
    <section class="anime-studio-home"><div class="anime-studio-hero"><div><small>TEGY · AI ANIME STUDIO</small><h2>What do you want to create?</h2><p>広告工程から独立して制作できます。必要な場合だけProject Script・Research・Assetsを接続します。</p></div><span>Studio mode · ${escapeHtml(mode)}</span></div><form id="animeWorkspaceForm" class="anime-studio-brief"><div class="anime-entry-modes">${modeCards.map(card=>`<label><input type="radio" name="productionMode" value="${card[0]}" ${mode===card[0]?'checked':''}><span><i>${card[1]}</i><b>${card[2]}</b><small>${card[3]}</small><em>${card[4]}</em></span></label>`).join('')}</div><div class="anime-studio-inputs"><label>Creation Source<select name="mode"><option value="auto">Generate story with Writer Agent</option><option value="manual">Use my story / script</option><option value="import" ${imported?'selected':''}>Connect Project Script</option></select></label><label>Duration<input name="durationSeconds" type="number" min="5" value="${treatment?.targetDurationSeconds || 30}"></label><label>Format<select name="aspectRatio"><option>9:16</option><option>16:9</option><option>1:1</option></select></label><label>Visual Direction<input name="style" value="${escapeHtml(treatment?.visualApproach || '2D anime · expressive character comedy')}"></label></div><label>Story / Episode Idea<textarea name="requirement" required placeholder="例：少しポンコツな新入社員が、初出勤で能力を隠そうとして失敗する30秒小劇場"></textarea></label><label>Existing Script / Series Note<textarea name="manualScript" placeholder="既存Script、Character設定、前Episodeの続きなど。Auto modeでは空欄でOK。"></textarea></label><button type="submit">Start Studio Team →</button></form><div class="anime-studio-team"><header><div><small>STUDIO TEAM</small><h3>Specialist Agents</h3></div><span>Director coordinates all outputs</span></header><div>${team.map(member=>`<article><i>${member[0]}</i><b>${member[1]}</b><small>${member[2]}</small><em>${treatment?'Synced':'Waiting'}</em></article>`).join('')}</div></div></section>
